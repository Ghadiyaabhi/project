...
class UserCreateView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response({str(key): ['field required']}, status=status.HTTP_400_BAD_REQUEST)
        
        if User.objects.filter(username=username).exists():
            return Response({'username': ["this user already exists"]}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"message": "user created", "user": res}, status=status.HTTP_201_CREATED)
